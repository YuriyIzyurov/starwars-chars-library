...
const calculatePosition = () => {
  ...
  const widthValues = getComputedStyle(card).getPropertyValue('--scaleX').match(/calc\(([\d.]+)px\/([\d.]+)px\)/)
  const heightValues = getComputedStyle(card).getPropertyValue('--scaleY').match(/calc\(([\d.]+)px\/([\d.]+)px\)/)

  const scaleX = widthValues[1]/widthValues[2]
  const scaleY = heightValues[1]/heightValues[2]
...
function startAnimation(element: HTMLDivElement | null = null, direction: AnimationType) {
  ...
  const speed = direction==='forward' ? 70 : 20
...
<style lang="scss">
  ...
  //пока хардкодом на глаз, нужно придумать решение получше
  .move-down {
